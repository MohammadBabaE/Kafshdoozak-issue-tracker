...
import { IssuesActions, setIssueLoading } from "./issuesSlice";
...
export const submitIssueVoteEpic = (
  action$: Observable<PayloadAction<SubmitIssueVotePayload>>
) =>
  action$.pipe(
    ofType(IssuesActions.submitIssueVote.type),
    switchMap((action: PayloadAction<SubmitIssueVotePayload>) => {
      ...
      if (action.payload.request === "CREATE") {
        return fromFetch(`/api/issues/${action.payload.issueId}/votes`, {
          ...
          switchMap((response) => {
            if (response.ok) {
              return of(setIssueLoading(true) ,IssuesActions.getSingleIssue({ id: action.payload.issueId }));
            } else {
              return EMPTY;
            }
          }),
